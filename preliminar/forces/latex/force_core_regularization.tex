...
\title{Problemas con la fuerza original y propuesta de regularización}
\author{}
\date{}
...
Lo interesante está en la elección de $\mathcal{F}[\xi_{ij}]$, que es una función que aparece en la energía de intereacción binaria:
...
En consecuencia, para $\gamma=1$ la rama decreciente comienza a una superposición considerablemente menor que para $\gamma=3$, algo que es raro ya que parecía que $\gamma=1$ era el máximo de la fuerza. 

Las superposiciones a las que esto pasa son altas, por lo que en sistemas diluidos o más "estándar" no parece haber problemas, pero en puntos donde la presión es muy alta, esta superposición sí se puede "vencer" y luego las células se terminan uniendo como vemos en las simulaciones.
...
Esta fuerza está graficada en la figura \ref{fig:gamma_dependence}. En el primer panel se ve la dependencia con $\gamma$ para una distancia fija $r=2R$, mientras que en el segundo panel se ve la dependencia con la distancia entre centros, tanto para $\gamma=1$ y $\gamma=3$. Para el caso del primer panel, se ve que $\gamma=1$ es el máximo, como ya habíamos visto. Sin embargo, en el segundo panel vemos que eso sólo pasa cuando las células se separan por esa distancia. En este panel también se puede ver cómo la fuerza decrece cuando la superposición es lo suficientemente alta.
...
Para cualquier $\lambda$ en el límite $\xi_{ij}\ll1$,

\begin{equation}
	\ln\left(1-\xi_{ij}^{\gamma}\right) \approx -\left(\xi_{ij}+\frac{\xi_{ij}^{2}}{2}+\frac{\xi_{ij}^{3}}{3}+...\right)
\end{equation}
...
Ahora, notemos en la figura \ref{fig:round_regularization} que pasa algo peculiar. Si $\lambda$ es muy chico, entonces se va a $\infty$ en el origen pero decrece un poco después del máximo de la fuerza original.
...
Por lo tanto, siempre que elijamos un $\lambda>\lambda_c$, estamos seguros de que la fuerza va a crecer siempre que las células se acerquen.
